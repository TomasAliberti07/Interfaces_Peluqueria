...
def iniciar_turnero():
    ...
    def modificar_turno():
        ...
        fecha = cal.get_date()
        hora = entry_hora.get().upper()  # Convertir hora a mayúsculas
        nombre_cliente = entry_cliente.get().upper()  # Convertir nombre_cliente a mayúsculas
        servicio = combo_servicio.get().upper()  # Convertir servicio a mayúsculas
    ...
    # Botón Volver
    button_volver = tk.Button(frame_botones, text="Volver", command=root.destroy, bg='#F7F7F7')  # Establecer el color de fondo del botón
